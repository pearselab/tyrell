% TS: things still outstanding
% WDP: Please ask them why the region-specific mobility terms have two entries per region (this is something I asked Ettie about on the call, if you remember). I think it's probably because there are two sets of mobility data in there (residential and average, I would guess) but the report supplement states it's only one of those two (unless I'm mis-reading it). You can see this issue creeping in to the supplementary table, where the region entries are double-indexed.

%COVID-19 R0 and environment

\documentclass[12pt,english,a4paper]{article}
\usepackage[utf8]{inputenc}
\usepackage{babel}
% WDP: I commented out the line below...
...
\clearpage
\section*{Abstract} %PNAS - max 250 words
...
% What we've done to address this issue. 
...
% UPDATE NUMBERS AFTER RUN WITH NEW TEMPERATURE!
...
% TS: Need to add an example here:
% e.g. "the mean monthly temperature in X state varies by Y degrees C between July and December, according to our model, this would require an average mobility reduction of Z% to mitigate".
% I can update this when numbers are finalised.
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{../figures/us-bayes-posterior.pdf}  
...
\label{fig:bayes}
\end{figure}
% TS: Need to make sure that this is calculated properly
