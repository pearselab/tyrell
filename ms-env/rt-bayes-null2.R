...
# Get output file name (note the above is copy-pasted from the original model for compatibility)
...
env <- apply(env, 2, median)
for(i in seq_len(nrow(env_time)))
    env_time[i,] <- .pad(env[as.character(processed_data$dates[[i]])], 115)
...
save(fit, dates, reported_cases, reported_deaths, states,
     ...
     file=paste0("results/rt-bayes-null2-",file_datestamp,".Rdata"))
