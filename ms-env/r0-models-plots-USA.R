...
USA_regression_model_full <- lm(R0 ~ Temperature + Absolute_Humidity + log10(Pop_density), data = USA_R0_data)
summary(USA_regression_model_full)
# print(xtable(summary(USA_regression_model_full)))
# scaled coefficicents:
xtable(summary(lm(R0 ~ scale(Temperature) + scale(Absolute_Humidity) + scale(log10(Pop_density)), data = USA_R0_data)))

# 2. check corellation between climate variables

USA_clim_vars <- USA_R0_data[,c("Temperature", "Absolute_Humidity")]
...
Rt_3d <- plot_ly() %>% 
  add_surface(x = ~log10(pops_ld), y = ~temps_ld, z = ~Rt, opacity = 0.9, cmin = 0, cmax = 4) %>%
  add_trace(x = log10(USA_Rt_data$Pop_density), 
...
heatmap_plot <- ggplot(predicted_R0, aes(x = Temperature, y = log10(Pop_density))) + 
  ...
  geom_point(data = USA_R0_data, aes(x = Temperature, y = log10(Pop_density), fill = R0), size = 4, shape = 21) +
  geom_text(data = USA_R0_data, aes(x = Temperature, y = log10(Pop_density), label = State), hjust = 0, vjust = 0, position = position_nudge(y = 0.05)) +
