data {
  ...
  matrix[N2, M] env_dat; // Environmental data for all days in each country
}
...
parameters {
  ...
  real env_slp; // Environmental effect
}
...
model {
  ...
  env_slp ~ normal(0, .1); // Added environment; strong-ish prior for 0 effect
  for(m in 1:M){
